...
public class AuthController {
    ...
    @PostMapping("/login")
    public ResponseEntity<ApiResult<TokenResponse>> login(@Valid @RequestBody LoginRequestDto loginDto) {
        log.info("로그인 요청: {}", loginDto.getEmail());
        TokenResponse tokenResponse = authService.login(loginDto);
        return ResponseEntity.ok(ApiResult.success("로그인이 성공했습니다.", tokenResponse));
    }
    ...
    @PostMapping("/refresh")
    public ResponseEntity<ApiResult<TokenResponse>> refreshToken(@RequestBody RefreshRequest request) {
        log.info("토큰 갱신 요청: refreshToken={}", request.getRefreshToken());
        TokenResponse tokenResponse = authService.refreshToken(request);
        return ResponseEntity.ok(ApiResult.success("토큰이 성공적으로 갱신되었습니다.", tokenResponse));
    }
}